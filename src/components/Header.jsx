          ...
          <header className="d-flex justify-content-around navbar transparent-background-fluid sticky-top mb-5">
            ...
            <div className="d-grid gap-2 d-md-flex justify-content-md-end animate__animated animate__fadeInRight">
              ...
              {isAuthenticated ?
                ...
                <Link to="/account" type="button" className={"btn btn-light btn-sm me-md-2 rounded-pill shadow fw-semibold " + (({ isActive }) => isActive ? "nav-link active" : "nav-link")} style={{ paddingLeft: 10, paddingRight: 10 }}><img src={user?.userProfileImg ? `data:image/png;base64,${user.userProfileImg}` : default_user_profile_img} width="25" height="25" style={{objectFit: "cover"}} alt="user-profile-img" className="rounded-circle border border-2 me-md-2" />{user.fullName?.length >= 20 ? user.fullName.substring(0, 25) + "..." : user.fullName}</Link>
                :
